...
from fpl import FPL
from fpltinker.models import Configuration
from fpltinker.data import configuration_data
...
async def update(app: Flask, fpl: FPL) -> dict:
    ...
    if not (app.config.get("FLAGS", {}).get("configuration")):
        return
    ...
    if not fpl_gameweeks[0].get("finished", True):
        ...
        details = configuration_data.get("season", None)
